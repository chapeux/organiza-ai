...
export default function DashboardView({
  ...
  const getDeadlineStatus = (demand: any) => {
    if (demand.status === "concluido" || demand.status === "concluído")
      return null;
    ...
    switch (sortBy) {
      ...
      case "deadline":
        const deadlineA = a.deadline
          ...
          : a.currentStep?.estimated_date
            ? new Date(a.currentStep.estimated_date).getTime()
            : Infinity;
        const deadlineB = b.deadline
          ? new Date(b.deadline).getTime()
          : b.currentStep?.estimated_date
    ...
    <div className="max-w-7xl mx-auto px-6 py-8 w-full">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            <h2 className="text-3xl font-extrabold text-primary tracking-tight mb-1 font-headline">
              Olá, {userName || "Arquiteto Industrial"}
            </h2>
            <p className="text-on-surface-variant font-medium">
              Aqui está o panorama atual das suas tarefas.
            </p>
          </div>
          <div className="flex items-center gap-4">
            <div className="bg-surface-container px-6 py-3 rounded-2xl text-center border border-outline-variant/20 shadow-sm min-w-[140px]">
              <p className="text-[10px] uppercase tracking-widest font-black text-outline mb-1">
                Total em Aberto
              </p>
            ...
            <div className="bg-primary/5 px-6 py-3 rounded-2xl text-center border border-primary/20 shadow-sm min-w-[140px]">
              <p className="text-[10px] uppercase tracking-widest font-black text-primary/70 mb-1">
                Total Concluídos
              </p>
              <p className="text-3xl font-headline font-black text-primary leading-none">
                {demands.filter((d) => d.status === "concluido").length}
      ...
      {!hasSupabase && (
        <div className="bg-error-container text-on-error-container p-4 rounded-xl mb-6">
          Banco de dados não configurado. Por favor, adicione suas credenciais
          do Supabase.
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6 pb-20">
        ...
        <div className="md:col-span-12 space-y-6">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <h3 className="text-xl font-bold tracking-tight text-primary font-headline">
              Resumo de Projetos e Demandas
            </h3>
          ...
          {loading ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {Array.from({ length: 3 }).map((_, i) => (
                <div
                  key={i}
                  className="bg-surface-container-lowest p-6 rounded-2xl shadow-sm border border-outline-variant/10 animate-pulse space-y-4"
                >
            ...
            <div className="text-on-surface-variant text-sm p-4 text-center">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {sortedDemands.map((demand) => (
                ...
                >
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex items-center gap-2">
                      {demand.status === "concluido" ? (
                        <span
                          className="material-symbols-outlined text-green-600 text-lg font-bold"
                        ...
                        <div
                          className={`w-2.5 h-2.5 rounded-full ${demand.priority === "critica" ? "bg-red-600" : demand.priority === "alta" ? "bg-orange-500" : demand.priority === "media" ? "bg-blue-500" : "bg-slate-400"}`}
                        ></div>
                      )}
                      <div
                      ...
                      {currentUserId &&
                        demand.user_id &&
                        currentUserId !== demand.user_id && (
                          <div
                            title={`Compartilhado por equipe`}
                            className="bg-primary/10 text-primary p-1 rounded-full flex items-center justify-center"
                          >
                            <Users size={14} />
                          </div>
                        )}
                    ...
                    >
                      ...
                      {openMenuId === demand.id && (
                        <div className="absolute right-0 mt-2 w-32 bg-surface-container-lowest rounded-xl shadow-lg border border-outline-variant/10 p-1 z-50">
                          <button
                            onClick={(e) => handleEditClick(demand, e)}
                            className="block w-full text-left px-3 py-2 text-sm text-on-surface hover:bg-surface-container-high rounded-lg"
                          >
                            Editar
                          </button>
                          <button
                            onClick={(e) => handleDelete(demand.id, e)}
                            className="block w-full text-left px-3 py-2 text-sm text-red-500 hover:bg-red-500/10 rounded-lg"
                          >
                            Excluir
                          </button>
                        </div>
                  ...
                  <h4 className="font-bold text-lg mb-1 leading-tight font-headline">
                    {demand.title}
                  </h4>
                  {getDeadlineStatus(demand) && (
                    <div
                      className={`mt-2 mb-3 px-3 py-1.5 rounded-lg border flex items-center gap-2 text-[11px] font-bold ${getDeadlineStatus(demand)?.color}`}
                    >
                      <span className="material-symbols-outlined text-[16px]">
                        {getDeadlineStatus(demand)?.icon}
                      </span>
                  ...
                  {demand.type === "project" && (
                    <p className="text-sm text-on-surface-variant mb-6">
                      Etapa: {(demand as any).currentStep?.label || "Iniciado"}
                    </p>
                  )}
                  {(demand.location ||
                    demand.deadline ||
                    (demand.workflow_steps?.length > 0 &&
                      demand.workflow_steps[0].estimated_date)) && (
                    ...
                    >
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          {demand.status === "concluido" ? (
                            <span className="material-symbols-outlined text-green-600 text-lg font-bold">
                              check_circle
                            </span>
                          ) : (
                            <div
                      ...
                      <td className="px-6 py-4 font-bold text-primary max-w-sm truncate">
                        <div className="flex flex-col gap-1">
                          ...
                          {getDeadlineStatus(demand) && (
                            <div
                              className={`w-fit px-2 py-0.5 rounded border flex items-center gap-1.5 text-[10px] font-black ${getDeadlineStatus(demand)?.color}`}
                            >
                              <span className="material-symbols-outlined text-[14px]">
                                {getDeadlineStatus(demand)?.icon}
                              </span>
