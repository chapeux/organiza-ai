...
export default function CalendarView() {
    ...
    while (day <= endDate) {
      for (let i = 0; i < 7; i++) {
        ...
        days.push(
          <div
            className={cn(
              "md:min-h-[140px] min-h-[100px] p-2 border-r border-b border-outline-variant/30 flex flex-col gap-1 cursor-pointer transition-colors",
              !isSameMonth(day, monthStart) ? "text-outline-variant bg-surface-container-lowest/50" : "bg-surface-container-lowest",
              isSameDay(day, selectedDate) ? "bg-primary/5" : "hover:bg-surface-container-low"
            )}
            key={day.toISOString()}
  ...
  return (
    <div className="flex flex-col xl:flex-row h-[calc(100vh-80px)] w-full overflow-hidden bg-surface">
         ...
         <div className="p-6 bg-surface-container-lowest border-b border-outline-variant/20 flex flex-col sm:flex-row sm:items-center justify-between gap-4 sticky top-0 z-10">
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center rounded-lg border border-outline-variant/50 bg-surface-container overflow-hidden">
                <button onClick={prevMonth} className="p-2 hover:bg-surface-container-highest transition-colors text-on-surface">
                  <ChevronLeft size={20} />
                </button>
                <div className="w-px h-5 bg-outline-variant/30" />
                <button onClick={nextMonth} className="p-2 hover:bg-surface-container-highest transition-colors text-on-surface">
      ...
      <div className="w-full xl:w-80 border-t xl:border-t-0 xl:border-l border-outline-variant/20 bg-surface-container-lowest p-6 flex flex-col xl:h-full xl:overflow-y-auto shrink-0">
         ...
         <div className="flex flex-col gap-4">
            {selectedDayEvents.map(evt => (
                <div key={evt.id} className="p-4 rounded-xl bg-surface border border-outline-variant/30 shadow-sm flex flex-col gap-2 relative overflow-hidden transition-shadow hover:shadow-md">
